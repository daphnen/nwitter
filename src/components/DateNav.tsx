...
export default function DateNav({
  ...
  return (
    <nav className="my-5 flex items-center justify-center gap-2.5">
      <Link href={`/?date=${shiftKey(date, -1)}${suffix}`} aria-label="이전 날" className={arrow}>
        ‹
      </Link>
      ...
      <Link href={`/?date=${shiftKey(date, 1)}${suffix}`} aria-label="다음 날" className={arrow}>
        ›
      </Link>
